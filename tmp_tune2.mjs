...
function show(name, rungs){
  let poly=[...SIMPLEX]; const rows=[]; const det=[]; let minBite=1;
  rungs.forEach((R,i)=>{
    const before=poly;
    for(const r of R){const b2=poly;for(const h of r.hs)poly=clip(poly,h);
      const cut=(area(b2)-area(poly))/whole; minBite=Math.min(minBite,cut);
      det.push(`   r${i} ${r.label}  cuts ${(cut*100).toFixed(1)}%  left ${((area(poly)/whole)*100).toFixed(1)}%`);}
    ...
    rows.push({rung:i, legal:+(area(poly)/whole*100).toFixed(1), verts:poly.length, ok:pr.inside,
      ...
      sellNvda:Math.round((w[0]-YOURS[0])*SIZES[i]),
      turnPP:+(t*100).toFixed(1), tradeDollars:Math.round(t*SIZES[i]),
      cg:toW(cent(poly)).map(v=>(v*100).toFixed(1)).join('/')});
  });
  console.log('\n===== '+name+`   [minBite ${(minBite*100).toFixed(1)}%]`); console.table(rows); console.log(det.join('\n'));
}
show('Q1 g>=20 | n<=40,b<=40 | n<=25,g>=45', [[],
  [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=40',hs:[capB(0.40)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=45',hs:[floorG(0.45)]}]]);
show('Q2 g>=20 | n<=40,b<=40 | n<=25,g>=50', [[],
  [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=40',hs:[capB(0.40)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=50',hs:[floorG(0.50)]}]]);
show('Q3 g>=20 | n<=40,b<=35 | n<=25,g>=50', [[],
  [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=35',hs:[capB(0.35)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=50',hs:[floorG(0.50)]}]]);
show('Q4 g>=20 | n<=40,b<=35 | n<=25,g>=45,risk<=0.30', [[],
  [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=35',hs:[capB(0.35)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=45',hs:[floorG(0.45)]},{label:'risk<=0.30',hs:[risk(0.30)]}]]);
show('Q5 g>=25 | n<=40,b<=35 | n<=25,g>=50', [[],
  [{label:'gold>=25',hs:[floorG(0.25)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=35',hs:[capB(0.35)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=50',hs:[floorG(0.50)]}]]);
