...
function show(name, rungs){
  let poly=[...SIMPLEX]; const rows=[]; const det=[];
  rungs.forEach((R,i)=>{
    const before=poly;
    for(const r of R){const b2=poly;for(const h of r.hs)poly=clip(poly,h);
      det.push(`   r${i} ${r.label}  cuts ${(((area(b2)-area(poly))/whole)*100).toFixed(1)}%  left ${((area(poly)/whole)*100).toFixed(1)}%  drawable ${r.hs.map(h=>!!cutLine(h)).join(',')}`);}
    const pr=project(toPlane(YOURS),poly); const w=toW(pr.p); const t=turnover(w,YOURS);
    rows.push({rung:i, legal:+(area(poly)/whole*100).toFixed(1), cutRung:+((area(before)-area(poly))/whole*100).toFixed(1),
      verts:poly.length, legalDot:pr.inside,
      nearest:w.map(v=>(v*100).toFixed(1)).join('/'),
      deltaPP:w.map((v,k)=>((v-YOURS[k])*100).toFixed(1)).join('/'),
      dollarMoves:w.map((v,k)=>Math.round((v-YOURS[k])*SIZES[i])).join('/'),
      turnPP:+(t*100).toFixed(1), tradeDollars:Math.round(t*SIZES[i]),
      cg:toW(cent(poly)).map(v=>(v*100).toFixed(1)).join('/')});
  });
  console.log('\n===== '+name); console.table(rows); console.log(det.join('\n'));
}

show('P1  g>=20 | n<=40,b<=35 | n<=25,g>=45', [
  [], [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=35',hs:[capB(0.35)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=45',hs:[floorG(0.45)]}],
]);
show('P2  g>=20 | n<=40,b<=35 | n<=25,g>=40', [
  [], [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=35',hs:[capB(0.35)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=40',hs:[floorG(0.40)]}],
]);
show('P3  g>=20 | n<=40,b<=30 | n<=25,g>=45', [
  [], [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=30',hs:[capB(0.30)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=45',hs:[floorG(0.45)]}],
]);
show('P4  g>=20 | n<=40,risk<=0.34 | n<=25,g>=45', [
  [], [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'risk<=0.34',hs:[risk(0.34)]}],
  [{label:'nvda<=25',hs:[capN(0.25)]},{label:'gold>=45',hs:[floorG(0.45)]}],
]);
show('P5  g>=20 | n<=40,b<=35 | n<=22,g>=45', [
  [], [{label:'gold>=20',hs:[floorG(0.20)]}],
  [{label:'nvda<=40',hs:[capN(0.40)]},{label:'bac<=35',hs:[capB(0.35)]}],
  [{label:'nvda<=22',hs:[capN(0.22)]},{label:'gold>=45',hs:[floorG(0.45)]}],
]);
