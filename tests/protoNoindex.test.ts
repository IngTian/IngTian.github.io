 ...
 *
 * THE BUG THIS EXISTS FOR. /proto-showpiece and /proto-sketches shipped without `noindex`, and
 * BaseLayout's head is a single either/or:
 *
 *     {noindex ? <meta name="robots" content="noindex" /> : <link rel="canonical" href={canonical} />}
 *
 * so the missing prop did not merely fail to discourage a crawler — it took the other branch and
 * emitted `rel="canonical" href="https://ingtian.github.io/proto-showpiece/"`, an explicit request to
 * index that URL, on a route whose rendered body carries internal work-in-progress (a scoring rubric,
 * "ranked #1/#2/#3" kill-gate notes, and the reasons five earlier attempts were rejected). robots.txt
 * is `Allow: /`, and the sitemap's exclusion does not help: a sitemap is a hint about what to fetch,
 * not a rule about what to keep, and a canonical is the stronger signal.
 *
 * WHY GLOB THE DIRECTORY instead of listing the four known routes. The failure mode is additive: the
 * proto routes are a workflow (a candidate costs one function and one page), so there WILL be a fifth,
...
describe('prototype routes are noindex at the source', () => {
  ...
  it('finds the proto routes at all (a rename must not silently empty this suite)', () => {
    // Without this, renaming the prefix would leave every it.each below with nothing to iterate and
    // the file would go green while checking nothing — the same shape of hole as `passWithNoTests`.
    expect(protoPages.length).toBeGreaterThanOrEqual(4);
    expect(protoPages).toContain('proto-showpiece.astro');
  ...
  it.each(protoPages)('%s passes noindex to BaseLayout', (file) => {
    ...
    // markup: the note next to proto-showpiece's own noindex contains the literal
